...
const ProximityLetter = ({ char, mouseX, mouseY }: { char: string; mouseX: any; mouseY: any }) => {
  ...
  useEffect(() => {
    ...
    function updatePosition(curX: number, curY: number) {
      ...
      const dist = Math.sqrt(Math.pow(curX - charX, 2) + Math.pow(curY - charY, 2));
      const radius = 150;

      if (dist < radius) {
        const power = (radius - dist) / radius;
        scale.set(1 + power * 0.4);
        const angle = Math.atan2(charY - curY, charX - curX);
        x.set(Math.cos(angle) * power * 8);
        y.set(Math.sin(angle) * power * 8);
        rotate.set(power * (Math.random() > 0.5 ? 5 : -5));
      } else {
        scale.set(1); x.set(0); y.set(0); rotate.set(0);
      }
    }
  ...
  return (
    <motion.span ref={ref} style={{ scale, x, y, rotate, display: "inline-block" }} className="will-change-transform">
      {char === " " ? "\u00A0" : char}
    </motion.span>
  );
};

const NavigationItem = ({ title, subtitle, href, mouseX, mouseY }: any) => (
  <Link href={href} className="group relative py-6 md:py-12 px-2 md:px-4 block transition-all duration-300">
    ...
    <div className="relative z-10">
      <div className="text-4xl md:text-7xl font-black tracking-tighter mb-2 md:mb-3 text-white whitespace-nowrap">
        {title.split("").map((char: any, i: number) => (
          <ProximityLetter key={i} char={char} mouseX={mouseX} mouseY={mouseY} />
        ))}
      </div>
      ...
      >
        <div className="max-w-7xl mx-auto w-full relative z-10">
          ...
          <nav className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4 md:gap-16">
            <NavigationItem title="BLOGS" subtitle="Articles" href="/sabha/blogs" mouseX={mouseX} mouseY={mouseY} />
            <NavigationItem title="FEED" subtitle="Community" href="/sabha/feed" mouseX={mouseX} mouseY={mouseY} />
            <NavigationItem title="PROJECTS" subtitle="Build" href="/sabha/projects" mouseX={mouseX} mouseY={mouseY} />
          </nav>
        </div>
      </section>
      <Footer />
